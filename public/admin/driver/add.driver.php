...
$driverID = $firstName = $lastName = $email = $password = $roleCd = $pImg = null;
$dbMutationStatus=null;

//import database utils
require_once(dirname(__DIR__) . "../../auth/authorization.php");
authorize('driverID', '../index.php');
require_once(dirname(__DIR__) . "../../common/utils.php");

function addNewDriver($firstName, $lastName, $email, $password, $hometown, $pImg) {
  $query = 
    "INSERT INTO driver (firstName, lastName, email, password, hometown) 
      VALUES ('$firstName', '$lastName', '$email', '$password', '$hometown')"
    ; //, profileImage ,'$pImg'
...
if( isset($_POST['addNewDriver']) || isset($_POST['updateDriver']) ) {
  ...
  $hometown = filter_var($_POST['hometown'], FILTER_SANITIZE_STRING);
  //$pImg = filter_var($_POST['profileImage'], FILTER_SANITIZE_STRING);
}

if( isset($_POST['addNewDriver']) ) {
  $dbMutationStatus = 
    addNewDriver($firstName, $lastName, $email, $password, $roleCd, null);
}
?>
...
<body class="bg-gray-200 antialiased font-sans">
  ...
  <div class="mt-16 py-8 px-6 mx-auto bg-white flex flex-wrap items-center w-full lg:w-4/5">
    ...
    <!-- Driver Form -->
    <form 
      id="addDriverForm" class="w-full mx-auto"  method="POST" action="./add.driver.php" >
      ...
      <div class="pr-4 mt-8 mb-4 w-full flex items-center justify-end">
        <button 
          type="submit" id="addNewDriver" name="addNewDriver"
