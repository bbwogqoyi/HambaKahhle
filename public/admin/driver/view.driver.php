...
$driverID = $firstName = $lastName = $email = $password = $licenseList = $pImg = null;
$dbMutationStatus=null;

if( isset($_POST['addNewDriver']) || isset($_POST['updateDriver']) ) {
  $firstName = filter_var($_POST['firstName'], FILTER_SANITIZE_STRING);
  $lastName = filter_var($_POST['lastName'], FILTER_SANITIZE_STRING);
  $email = filter_var($_POST['email'], FILTER_SANITIZE_STRING);
  $password = filter_var($_POST['password'], FILTER_SANITIZE_STRING);
  $roleCd = filter_var($_POST['DriverTypeID'], FILTER_SANITIZE_STRING);
  //$pImg = filter_var($_POST['profileImage'], FILTER_SANITIZE_STRING);
}

function retrieveExistingDriver($driverID) {
  $query = "SELECT * from Driver e where e.driverID='$driverID'";
  return executeQuery($query);
}

function deleteExistingDriver($driverID) {
  $query = "DELETE FROM Driver e WHERE e.driverID='$driverID'";
  return executeQuery($query);
}

if( isset($_POST['deleteDriver']) ) {
  deleteExistingDriver($_REQUEST['driverID']);
  header("Location: ../assets/drivers.php");
}
?>
...
<body class="bg-gray-200 antialiased font-sans">
  ...
  <div 
    ...
    <script>
      ...
      function deleteDepotRecord() {
        document.getElementById("updateDriverForm").submit();
      }
    </script>
  </div>
  ...
  <div class="mt-16 py-8 px-6 mx-auto bg-white flex flex-wrap items-center w-full lg:w-4/5">
    ...
    <!-- Driver Form -->
    <form 
      id="updateDriverForm" class="w-full mx-auto"  method="POST" 
      <?php 

        $queryParam = ( isset($_REQUEST['driverID']) ? '?driverID='.$_REQUEST['driverID'] : '');

        echo 'action="./view.driver.php'. $queryParam .'"';
      ?>
    >
      ...
      <div class="w-full flex flex-row justify-between items-center">
        ...
        <?php
          $result = retrieveExistingDriver($_REQUEST['driverID']);
        ...
        <div class="w-2/3 flex flex-col">
          ...
          <div class="flex w-full mb-8">
            <div class="form-group w-full md:w-1/2 px-3 mb-6 ">
              ...
              <input
                <?php echo ( 'disabled value="'. $driver['firstName'] .'"'  ); ?> 
            ...
            <div class="form-group w-full md:w-1/2 px-3 mb-6 ">
              ...
              <input
                <?php echo ( 'disabled value="'. $driver['lastName'] .'"'  ); ?> 
          ...
          <div class="flex w-full mb-8">
            <div class="form-group w-full md:w-1/2 px-3 mb-6 ">
              ...
              <input 
                <?php echo ( 'disabled value="'. $driver['email'] .'"'  ); ?> 
            ...
            <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
              ...
              <div class="form-group relative">
                <select 
                  ...
                  <option value=""></option>
                  <option <?php echo ( $driver['employeeTypeID'] == "1" ? "selected" : "" ); ?> value="1">Admin Staff</option>
                  <option <?php echo ( $driver['employeeTypeID'] == "2" ? "selected" : "" ); ?> value="2">HR Staff</option>
                  <option <?php echo ( $driver['employeeTypeID'] == "3" ? "selected" : "" ); ?> value="3">Managers</option>
                </select>
          ...
          <div class="flex w-full mb-8">
            <div class="form-group w-full px-3 mb-6 ">
              ...
              <input 
                <?php echo ( 'disabled value="'. $driver['password'] .'"'  ); ?> 
      ...
      <div class="pr-4 mt-8 mb-4 w-full flex items-center justify-end">
        <a 
          <?php echo 'href="./update.driver.php?driverID='.$_REQUEST['driverID'].'"'; ?>
          class="bg-indigo-400 hover:bg-indigo-700 text-white font-bold text-center text-lg py-4 px-auto w-56 mr-2 rounded">
            Edit Driver
        </a>
        <input type="text" id="deleteDriver" name="deleteDriver" class="hidden" />
        <button 
          type="button"
          onClick="showModal();"
  ...
  <script>
    window.onload = function () {
      var form = document.getElementById("updateDriverForm");
