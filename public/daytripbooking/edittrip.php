 ...
 if(isset($_REQUEST['submit'])){
    $tripDate =  $_REQUEST['tripDate'];
		...
		$tripID =  $_REQUEST['id'];
			...
			$result = mysqli_query($conn, $query) or die("Oops, something went wrong, Could not Edit trip") ;
			...
			echo " <script type='text/javascript'>alert('failed to add!.');</script>";
				...
				echo " <script type='text/javascript'>alert('Edited daytrip successfully!.');</script>";
				header("Location: ../daytripdashboard/daytripdashboard.php?id=" . $_REQUEST['bid']);
			}
			mysqli_close($conn);
	}
...
<body class="antialiased bg-gray-200">
  ...
  <div class="mt-16 py-8 px-6 mx-auto bg-white flex flex-wrap items-center w-full lg:w-4/5">
    ...
    <!-- Content Table -->
              <?php
			 echo " <div class=\"w-full inline-flex w-full justify-between px-3\" style=\"left: 50%;padding-top: 0.75rem;padding-bottom: 0.75rem;right: 5px;padding-left: 0.75rem;\">
			    

<form class=\"content-around w-full shadow-2xl items-center\" style=\"padding-top: 0.75rem;padding-bottom: 0.75rem;right: 5px;padding-left: 0.75rem;\" action=\"edittrip.php?id=" . $_REQUEST['id'] . "&bid=" .  $_REQUEST['bid']  . "\" method=\"POST\">
  ...
  <div class=\"flex flex-wrap -mx-3 mb-6\">
	...
	if(!isset($_REQUEST['submit'])){
		...
		$result = mysqli_query($conn, $query) or die("Oops, something went wrong, Could not load trips") ;
		if($result === false) {
			echo " <script type='text/javascript'>alert('failed to add!.');</script>";
							...
							<div class=\"w-full md:w-1/2 px-3\">
							  <label class=\"block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2\"  style=\"padding-top: 0.75rem;padding-bottom: 0.75rem;right: 5px;padding-left: 0.75rem;\">
							   Pick up time
														...
														<input type=\"time\" id=\"tripTime\" name=\"tripTime\"  value=" . $row["tripTime"] . "></div>
						  </div>
						  <div class=\"flex flex-wrap -mx-3 mb-6\">
							<div class=\"w-full px-3\">
							  <label class=\"block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2\" style=\"padding-top: 0.75rem;padding-bottom: 0.75rem;right: 5px;padding-left: 0.75rem;\">
								Collection Point 
														...
														<div style=\"width: 10%;\"></div>
														<input type=\"text\" id=\"pickLocation\" name=\"pickLocation\"  value=" . $row["collectionPoint"] . ">
							</div>
						  </div>
						 <div class=\"flex flex-wrap -mx-3 mb-6\">
							  <label class=\"block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2\"  style=\"padding-top: 0.75rem;padding-bottom: 0.75rem;right: 5px;padding-left: 0.75rem;\">
